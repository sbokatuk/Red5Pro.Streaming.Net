...
# Selects the Xcode carrying the iOS SDK our net10 target framework is built against.
#
...
# against an Xcode carrying the matching iOS SDK:
#
#     error : This version of .NET for iOS (26.0.x) requires Xcode 26.0.
#             The current version of Xcode is 26.5.
#
# The runner image default drifts, so without this step the iOS device test may be unable to build
# the very target framework the package ships. Selecting explicitly also means an image update that
# moves the default Xcode cannot silently change what the build produces.
#
...
# Newest patch release first, so 26.0.10 wins over 26.0.9 rather than sorting lexically.
