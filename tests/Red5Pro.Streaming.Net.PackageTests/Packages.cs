...
{
    public const string Android = "Red5Pro.Streaming.Net.Android";
    public const string IOS = "Red5Pro.Streaming.Net.iOS";
    public const string Core = "Red5Pro.Streaming.Net";
    public const string Maui = "Red5Pro.Streaming.Net.Maui";

    /// <summary>
    /// Target frameworks the Android package must carry, one set per SDK band pass. Pinned rather
    /// than discovered: a package that silently lost a target framework because a pack pass failed
    /// or the merge dropped it is exactly the regression these tests exist to catch.
    /// </summary>
    ...
    /// <summary>
    /// The Apple packages are only built on macOS, so on a Linux run the iOS, core and MAUI tests
    /// skip rather than fail. CI validates on a runner that has both sets downloaded.
    /// </summary>
    ...
    {
        // The id is a filename prefix, and Red5Pro.Streaming.Net is a prefix of
        // Red5Pro.Streaming.Net.Android, so the version must be matched too or the core package
        // lookup would find all four.
    ...
    private static string ResolveArtifactsDirectory()
    {
        ...
        // A relative RED5_ARTIFACTS is resolved against the repository root, not the current
        // directory. The test process runs from bin/<config>/<tfm>, so the obvious-looking
        // RED5_ARTIFACTS=artifacts would otherwise point inside the build output.
        return Environment.GetEnvironmentVariable("RED5_ARTIFACTS") is { Length: > 0 } configured
