...
advertises the new target framework. Everything else comes from PRIMARY unchanged - which matters
here, because the native payload (native/webrtc-android.aar, native/*.xcframework) is packed once
at the package root rather than per target framework and so must not be duplicated by the merge.
...
def target_frameworks(package: zipfile.ZipFile) -> set[str]:
    ...
    Usually read from the lib/<tfm>/ folders, but a dependency-only package has no lib/ at all and
    declares its target frameworks solely as nuspec dependency groups. Ignoring those would
    silently drop a whole band during the merge.
    """
    found = set()
    for name in package.namelist():
        parts = name.split("/")
...
def add_dependency_groups(nuspec: str, frameworks: list[str], source: dict[str, str]) -> str:
    ...
    Edited as text rather than via ElementTree so that the rest of the nuspec - including attribute
    order and the xmlns declaration NuGet emits - is preserved byte for byte.
    """
    groups = "".join(
